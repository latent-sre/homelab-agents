...
**When generation fails naming a rewrite id, you reworded a sentence a host projection anchors
on**: `fleet/hosts/table.py` states the count each rewrite must land, so re-anchor it there
rather than regenerating past the message — the correction it carries is the only thing keeping a
Claude-only authority claim out of a host that cannot honour it.
