...
Claude's source profile preloads `code-craft`. This host cannot encode that
...
Every tool ships with its operational surface. The mechanics — observability, timeouts and retries, config from environment, secrets — are the applicable craft skills' rules (`backend-craft` for services and workers, `frontend-craft` for failure-state UX); don't re-derive them. What the crafts don't carry, you still owe on every tool, including CLIs and scripts:
...
Red flags — if you catch yourself thinking any of these, stop and verify — or load and work `root-cause` as directed above — instead:
