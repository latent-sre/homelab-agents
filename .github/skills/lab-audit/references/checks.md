...
- Fix class: restore the intended native service/job policy through `homelab-engineer`;
...
- Fix class: establish intended state, then reconcile the incorrect side. If runtime is correct,
  update the repo (Tier 1); apply through `homelab-engineer` only when runtime must
...
- For native hosts, compare OS release, patch/package-source state and intended update policy
  ...
  `researcher`; unavailable evidence leaves support status unverified. Do not install
