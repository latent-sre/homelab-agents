...
`homelab-engineer`'s authority rules: a bounded user request covers in-scope execution,
...
2. **Check the upgrade path.** Use upstream release/migration guidance covering the deployed →
   ...
   `researcher`, without private configuration or credentials.
...
After a storage or recovery-path change, use `restore-drill` to prove the affected path
...
recovery gets a short note; use `postmortem` when recovery exposed a reusable gap or a
