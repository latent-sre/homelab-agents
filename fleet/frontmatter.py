...
def yaml_scalar(value: str) -> str:
    ...
    """
    return _escape_line_breakers(json.dumps(value, ensure_ascii=False))


# NEL, LINE SEPARATOR, PARAGRAPH SEPARATOR. `json.dumps(..., ensure_ascii=False)` emits all three
# literally -- they are not JSON control characters -- but `str.splitlines()` breaks on every one,
# and this dialect reads frontmatter line by line. A value carrying one would therefore be written
# whole and read back torn in half, with the tail of the description parsed as a frontmatter key.
# Escaping them keeps the value intact for the readers that matter; no canonical value contains one
# today, which is why this changes no generated byte. Found by the property test over this dialect,
# not by a defect report: `str.splitlines()` and YAML 1.2 disagree about what ends a line, and that
# disagreement is invisible until someone pastes text from a word processor into a description.
_LINE_BREAKERS = {"\x85": "\\u0085", "\u2028": "\\u2028", "\u2029": "\\u2029"}


def _escape_line_breakers(rendered: str) -> str:
    for character, escape in _LINE_BREAKERS.items():
