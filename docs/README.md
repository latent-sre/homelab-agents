...
| [`decisions/2026-09-13-machinery-rewrite.md`](decisions/2026-09-13-machinery-rewrite.md) | Proposed; phase 0 implemented | The from-scratch design for the maintainer machinery: the duplicated primitives each audit found (with the live divergence in each), the `fleet/` kernel and its binding rules, why the frontmatter dialect stays hand-written with a conforming parser as tripwire, the dependency policy, the phase table with each phase's oracle, and the retirement of the routing runner to native `claude plugin eval` |
