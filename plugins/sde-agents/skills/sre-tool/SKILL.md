...
an obvious owner and established pattern directly to `sde-fullstack` or the matching
...
6. For anything network-exposed or auth-bearing, add a **security review** before deploy artifacts
   ship — spawn `application-security-auditor` on the tool's repository: whole-surface
   ...
   spawned, fall back to a second `code-reviewer` pass seeded with a security-only threat
